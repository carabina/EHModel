...
#ifndef EHDB_LOG
#define EHDB_LOG(code) _log(__LINE__, code, sqlite3_errmsg(self.db))
#endif
...
@implementation EHDatabase

- (void)syncInDb:(void (^)(EHDatabase *db))block {
...
- (void)asyncInDb:(void (^)(EHDatabase *db))block {
    if (dispatch_get_specific(self.queueKey)) {
        block(self);
    } else {
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create("com.code4god.EHModel.EHDatabase", NULL);
...
- (BOOL)open {
    sqlite3 *db;
    if (EHDB_LOG(sqlite3_open([self.file cStringUsingEncoding:NSUTF8StringEncoding], &db)) != SQLITE_OK) {
        return NO;
    }
    self.db = db;
    return YES;
...
- (BOOL)close {
    if (self.db) {
        if (EHDB_LOG(sqlite3_close(self.db)) != SQLITE_OK) {
            return NO;
        }
        self.db = nil;
    }
...
- (sqlite3_stmt *)stmtForSql:(NSString *)sql {
    EHStmt *s = self.stmts[sql];
    if (!s) {
        sqlite3_stmt *stmt = NULL;
        if (EHDB_LOG(sqlite3_prepare_v2(self.db, [sql UTF8String], -1, &stmt, 0)) != SQLITE_OK) {
    ...
    EHDB_LOG(sqlite3_reset(s.stmt));
    EHDB_LOG(sqlite3_clear_bindings(s.stmt));
    return s.stmt;
}
...
- (void)removeAllStmt {
    [self.stmts enumerateKeysAndObjectsUsingBlock:^(id _Nonnull key, EHStmt *_Nonnull obj, BOOL *_Nonnull stop) {
        if (obj.stmt) {
...
- (void)executeQuery:(NSString *)sql stmtBlock:(void (^)(sqlite3_stmt *stmt, int idx))stmtBlock resultBlock:(void (^)(sqlite3_stmt *stmt, bool *stop))resultBlock {
    ...
    bool stop = NO;
    while (EHDB_LOG(sqlite3_step(stmt)) == SQLITE_ROW) {
        resultBlock(stmt, &stop);
        if (stop) {
            return;
        }
...
- (BOOL)executeUpdate:(NSString *)sql stmtBlock:(void (^)(sqlite3_stmt *stmt, int idx))stmtBlock {
    ...
    if (EHDB_LOG(sqlite3_step(stmt)) != SQLITE_DONE) {
        return NO;
    }
    return YES;
}
